...
class OtherController {
  ...
  uploadImg = (req: Request, res: Response) => {
    ...
    const imgPath = `${process.env.IMAGE_PATH}/${req.file.filename}`;
    unifiedResponseBody({
      result_msg: "上传图片成功",
      result: {
        imgURL: imgPath,
  ...
  deleteImg = async (req: Request, res: Response) => {
    ...
    const imgPath = imgURL.split("/")[2];
    const path = `${process.env.IMAGE_PATH}/${imgPath}`;
    try {
      unifiedResponseBody({
        result_msg: "删除图片成功",
        result: { path },
